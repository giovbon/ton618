  ...
  return {
    ...
    items: ({ query }: { query: string }) => {
      const commands: CommandItem[] = [
        ...
        {
          title: "Imagem",
          description: "Upload de imagem para a nota.",
          icon: '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>',
          command: ({ editor, range }) => {
            editor.chain().focus().deleteRange(range).run();
            // Dispara evento para o editor abrir o input de arquivo
            window.dispatchEvent(new CustomEvent("tiptap:request-image", { detail: { editorId: editor.options.editorId } }));
          },
        },
        {
          title: "PDF",
          description: "Upload de documento PDF.",
          icon: '<svg class="w-4 h-4 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>',
          command: ({ editor, range }) => {
            editor.chain().focus().deleteRange(range).run();
            window.dispatchEvent(new CustomEvent("tiptap:request-pdf", { detail: { editorId: editor.options.editorId } }));
          },
        }
      ];
