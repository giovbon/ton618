...
## 8. Estrutura Monorepo (Open Core + Comercial)

📍 Decidido em: 20/07/2026

O projeto foi reestruturado para um modelo **open core**: o código base (core) é auto-hospedável via Docker, e funcionalidades comerciais são adicionadas via pastas separadas e build tags.

```
ton618/
├── core/                   ← Código compartilhado (auto-hospedado + comercial)
│   ├── cmd/server/         ← Servidor Go (chi + SQLite + templ)
│   ├── internal/           ← Toda a lógica de domínio
│   ├── web/                ← Frontend (TipTap, esbuild, Tailwind)
│   ├── go.mod              ← module ton618/core
│   └── Dockerfile          ← Gera imagem só com o core
│
├── desktop/                ← App desktop (Wails)
├── pkg/commercial/         ← Licenciamento RSA (módulo separado)
├── Makefile                ← Comandos unificados
└── docker-compose.yml      ← Aponta para core/
```

**Regras:**
- Correções de bug em `core/` beneficiam **tanto o auto-hospedado quanto o comercial**
- Features comerciais vão em `pkg/commercial/` ou `desktop/`, protegidas por build tag `commercial`
- O Dockerfile do core **nunca** inclui código comercial
- O workflow do GitHub (`deploy_core.yml`) builda apenas o `core/`

**Build comercial:**
```bash
docker build --build-arg BUILD_TAGS="sqlite_fts5 commercial" -t ton618-pro core/
```

## 9. Multi-Tenant (SaaS)

📍 `core/internal/middleware/tenant.go` — criado em 20/07/2026

Para operar como SaaS (um servidor atendendo vários clientes), implementou-se um **middleware de tenant** que isola bancos SQLite por subdomínio:

```
clienteA.ton618.io → /data/tenants/clienteA/ton618.db
clienteB.ton618.io → /data/tenants/clienteB/ton618.db
```

### Arquitetura

- **TenantManager**: struct thread-safe com cache de `*db.Store` por tenant
- **Middleware HTTP**: extrai o subdomínio do `Host`, cria ou recupera o banco, injeta no `context.Context`
- **Sem locks globais**: cada tenant tem seu próprio `WriteMu` (serialização por banco)
- **Criação lazy**: o banco só é criado no primeiro acesso do tenant

### Uso

```go
manager := middleware.NewTenantManager("/data/tenants")
handler := manager.Middleware(router)
```

### Considerações
- O middleware retorna `nil` em modo single-tenant (sem subdomínio) — o servidor funciona normalmente
- A função `extractTenantFromHost()` ignora `localhost`, `www` e domínios sem subdomínio
- O custo é linear com o número de tenants ativos simultaneamente (cada um com seu SQLite em WAL)

## 10. Licenciamento Comercial

📍 `pkg/commercial/license.go` — criado em 20/07/2026

Sistema de licenciamento para o produto comercial, baseado em **assinatura RSA-4096**.

### Estrutura

| Arquivo | Função |
|---------|--------|
| `pkg/commercial/license.go` | Validação de licença, planos, features |
| `pkg/commercial/cmd/genkey/main.go` | CLI para gerar chaves e licenças |
| `core/cmd/server/commercial_init.go` | Integração no servidor (build tag `commercial`) |
...
```bash
# 1. Gera chave privada + pública + licença de exemplo
cd pkg/commercial && go run ./cmd/genkey/ ./chaves/

# 2. Build comercial
docker build --build-arg BUILD_TAGS="sqlite_fts5 commercial" -t ton618-pro core/

# 3. Executa com vars de ambiente
docker run -e TON_LICENSE_PUB_KEY=/app/license.pub \
           -e TON_LICENSE_FILE=/app/license.lic \
           -v ./chaves/license.pub:/app/license.pub:ro \
           -v ./chaves/license.lic:/app/license.lic:ro \
           ton618-pro
```

### Segurança
- Licença assinada com RSA-4096 (PKCS#1v15 + SHA256)
- Chave privada fica apenas com o emissor (nunca no binário)
- Chave pública embutida no ambiente do servidor
- Licença contém: usuário, email, plano, expiry, features, max_tenants
- Qualquer tentativa de falsificação é detectada na validação da assinatura