...
|------------|--------|----------|
| Servidor HTTP | ✅ OK | `go build` + `go vet` + 9 pacotes de teste passando |
| API REST | ✅ OK | 12 endpoints: search, file, sync, upload, graph, settings, backup |
...
| Ollama embeddings | ✅ OK | Circuit breaker tri-state |
...
| DocsPanel | ✅ OK | Documentação interna via Markdown, botão 📖, SVGs reais dos botões |
...
TON618/
├── etl/                    # Backend Go
│   ├── main.go             # Entry point, server bootstrap, rotas
│   ├── go.mod              # Go 1.25.0
│   └── internal/
...
│       ├── semantic/       # Ollama, circuit breaker, cache
...
│   │   │   ├── WeightsSettings.tsx # Tela de configurações
...
|---|--------|-------|----------|
| 01 | `log.Printf` em vez de `slog` | `api/*.go`, `ingest/*.go` | ~50+ ocorrências em produção |
...
### Go Tests — ✅ Todos passando

```
etl                          0.006s
etl/internal/api             0.854s
etl/internal/clustering      0.038s
etl/internal/config          0.011s
etl/internal/ingest          7.745s
...
| 2026-05-05 | **Análise de viabilidade Wails v2** adicionada ao `context.md`. Conclusão: abordagem híbrida recomendada (8-10 dias), mas usuário optou por não fazer. |
...
| 2026-05-04 | **Circuit breaker Ollama** (tri-state: closed/open/half-open). **Migração go-readability** de pseudo-version para fork mantido. **Testes de corrupção BBolt** (4 cenários) + recuperação automática. |
...
| 2026-02-05 | **state.go fatiado** (state_tags, state_links, state_vectors criados). Injeção de dependência (GlobalCoordinator removido). Rate limiting middleware. fsnotify watcher. |
